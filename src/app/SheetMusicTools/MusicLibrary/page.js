  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white to-gray-300">
      ...
      {filteredFiles.length === 0 ? (
        ...
        <table className="musiclibrary border-b w-full shadow-xl overflow-hidden">
          <tbody>
            {pieceArray(filteredFiles, 3).map((row, rowIndex) => (
              <tr key={rowIndex}>
                  ...
                  <td className="musiclibrarydata hover:scale-105 font-bold hover:underline" key={file}>
                    <div className="flex flex-col items-center">
                      <a href={`/SheetMusicTools/MusicLibrary/MusicListener/${uid}/${file}`}>
                        <img src={'/musicnote.png'} className="w-60 h-60 mb-4" />
                      </a>
                ...
                {row.length < 3 &&
                  Array(3 - row.length).fill(null).map((_, index) => (
                    <td key={`empty-${index}`} className="px-10 mb-6 border-b"></td>
                  ))}
              </tr>
            ))}
          </tbody>
